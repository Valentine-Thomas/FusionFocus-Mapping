//Macro to isolate proteins centroids in the case were round signal used as references and a round POI.
//SplitChannelMacro adapted from Dr. A.Guevarra, align frames adapted from Pr A.Vejstica, BGN-MD from Dr. A.Picco
...
Dialog.create("Image correction, parameters:");
	...
	Dialog.addNumber("Median filter Radius (spot radius):",13);
   ...
   File.makeDirectory(outputBlue);  //create output folder
    ...
    //split channels   
        for (i=0; i<data.length; i++) {   // set algorithm for alfiles of folder
               ...
               open(inputPath); // open image i of the input folder
